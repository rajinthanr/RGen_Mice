...
public:
  ...
  void search_to(Location target)
  {
    ...
    motion.move(BACK_WALL_TO_CENTER, SEARCH_SPEED, SEARCH_SPEED, SEARCH_ACCELERATION);
    motion.set_position(HALF_CELL);
    printf("Off we go...[%d,%d]\n", target.x, target.y);
    motion.wait_until_position(SENSING_POSITION);
    while (m_location != target)
    {
      //      if (switches.button_pressed())
      //      { // allow user to abort gracefully
      //        break;
      //      }
      printf("\n");
      // reporter.log_action_status('-', ' ', m_location, m_heading);
  ...
  {
    int side = sensors.wait_for_user_start(); // cover front sensor with hand
  ...
  void conf_edge_detection()
  {
    ...
    if (left_edge_found)
    {
      printf("%d", BACK_WALL_TO_CENTER + left_edge_position);
    }
    else
    {
      printf("-");
    ...
    if (right_edge_found)
    {
      printf("%d", BACK_WALL_TO_CENTER + right_edge_position);
    }
    else
    {
      printf("-");
