...
bool Location::is_in_maze() { return x < MAZE_WIDTH && y < MAZE_HEIGHT; }
...
Heading ahead_from(const Heading heading) { return heading; }
...
bool Maze::is_exit(const Location cell, const Heading heading) const {
  ...
  switch (heading) {
  case NORTH:
    return (walls.north & m_mask) == EXIT;
  case EAST:
    return (walls.east & m_mask) == EXIT;
  case SOUTH:
    return (walls.south & m_mask) == EXIT;
  case WEST:
    return (walls.west & m_mask) == EXIT;
  default:
    return false;
  }
}

void Maze::update_wall_state(const Location cell, const Heading heading,
                             const WallState state) {

  //  switch (heading) {
  //      case NORTH: if ((m_walls[cell.x][cell.y].north & UNKNOWN) != UNKNOWN)
  //      return; break; case EAST:  if ((m_walls[cell.x][cell.y].east  &
  //      UNKNOWN) != UNKNOWN) return; break; case WEST:  if
  //      ((m_walls[cell.x][cell.y].west  & UNKNOWN) != UNKNOWN) return; break;
  //      case SOUTH: if ((m_walls[cell.x][cell.y].south & UNKNOWN) != UNKNOWN)
...
uint16_t Maze::neighbour_cost(const Location cell,
...
Heading Maze::heading_to_smallest(const Location cell,
  ...
  if (cost_val < best_cost && is_exit(cell, next_heading)) {
    best_cost = cost_val;
...
void Maze::set_wall_state(const Location loc, const Heading heading,
                          const WallState state) {
  switch (heading) {
  case NORTH:
