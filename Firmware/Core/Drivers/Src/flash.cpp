...
void commit_flash()
{
    ...
    uint32_t current_time = HAL_GetTick(); // Assuming HAL_GetTick() returns time in milliseconds
...
void read_flash()
{
    Flash_Read_Data(flash_data, NUM_WORDS);
}
...
{
    flash_data[index] = *(int *)&value;
}

int getInt(int index)
{
    return *(int *)&flash_data[index];
}
