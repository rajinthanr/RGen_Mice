...
void icm_initialize() {
  ...
  uint8_t accel_fs_sel = 0x40; // ACCEL_CONFIG: FS_SEL=2 (±8g), bits [3:2]=10
  uint8_t gyro_fs_sel =
      0x00; // GYRO_CONFIG: FS_SEL=0 (±2000 dps), bits [4:3]=00
  ...
  uint8_t whoami_reg =
      0x75 | 0x80; // WHO_AM_I register address with read bit set
...
void read_values() {
  ...
  // Accelerometer: convert raw values to m/s^2 (assuming ±4g, 16-bit, 1g
  // = 9.80665 m/s^2) Sensitivity for ±4g: 8192 LSB/g
