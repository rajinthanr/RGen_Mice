...
float wallFront() {
  ...
  float error_theta =
      atan((error_r - error_l) / FRONT_SENSOR_SPACING) * 180 / PI;
  ...
  float correction_angle = pid_theta.kp * error_theta +
                           pid_theta.ki * pid_theta.integral +
                           pid_theta.kd * derivative;

  correction_angle = clamp(correction_angle, -STEERING_ADJUST_LIMIT * 10,
                           STEERING_ADJUST_LIMIT * 10);
