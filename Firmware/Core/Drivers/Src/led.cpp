...
static const LedConfig led_configs[] = {{LED1_GPIO_Port, LED1_Pin},
...
void LED_On(uint8_t led_num) {
  if (led_num == 0 || led_num > sizeof(led_configs) / sizeof(led_configs[0]))
    return;
  HAL_GPIO_WritePin(led_configs[led_num - 1].port, led_configs[led_num - 1].pin,
                    GPIO_PIN_SET);
}
...
void LED_Toggle(uint8_t led_num) {
  if (led_num == 0 || led_num > sizeof(led_configs) / sizeof(led_configs[0]))
    return;
  HAL_GPIO_TogglePin(led_configs[led_num - 1].port,
                     led_configs[led_num - 1].pin);
}
...
void LED_Blink(uint8_t led_num, uint32_t delay_time, uint8_t times) {
  ...
  for (uint8_t i = 0; i < times; i++) {
    HAL_GPIO_WritePin(led_configs[led_num - 1].port,
                      led_configs[led_num - 1].pin, GPIO_PIN_SET);
    delay_ms(delay_time);
    HAL_GPIO_WritePin(led_configs[led_num - 1].port,
                      led_configs[led_num - 1].pin, GPIO_PIN_RESET);
    delay_ms(delay_time);
  }
}
